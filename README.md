...
- **管理后台** - 提供商品 CRUD 和 CSV 批量导入能力，满足后台维护需求

### 生产级特性

- 认证授权（PyJWT）、接口限流（flask-limiter）、安全响应头与 ProxyFix 真实 IP 识别
- `pydantic-settings` 统一配置、`loguru` 结构化日志按天轮转
- `pytest` 完整测试覆盖、GitHub Actions CI（测试 + Docker 镜像构建）
- Docker 三服务编排、Nginx 反向代理、可选 PostgreSQL（prod profile）
...
> 说明：如果你是首次接触本项目，建议优先使用 Docker Compose 启动；如果需要本地调试，也可以采用手动方式启动前后端与模型服务。
