...
1. **几何规则脸型分类（零训练数据依赖）** — 弃用需大量标注数据的 CNN 训练方案（历史 `face_shape_model.pth` 为噪声训练产物，已归档弃用），改用 MediaPipe Face Mesh 468 个面部关键点的几何比例（脸长宽比 / 下颌宽度 / 额头比例）做规则分类。整套分类阈值与「脸型 → 镜框」映射表为项目自研，无需任何标注数据集、无 GPU 即可运行。
2. **可解释规则推荐引擎** — 弃用伪标签决策树（`recommend_model.pkl` 为 4KB 占位模型，已弃用），自研「脸型 → 镜框映射 + 度数硬过滤 + 折射率 / 瞳距加权打分」规则引擎。每条推荐都返回**命中的具体规则**，用户与商家都能看懂「为什么推荐这副眼镜」，是面向导购场景的透明 AI。
3. **贴合式虚拟试戴几何算法** — 试戴框尺寸由「双眼距 × 2.4」推导、以双眼连线中点定位、按连线夹角旋转贴合，相比「居中贴图」更真实贴合不同脸型与头部姿态。
4. **拍照 → 识别 → 推荐 → 试戴 → 选购全闭环** — 多数同类学生作品只做单点（仅脸型分类或仅推荐），本项目打通从用户上传照片到下单的完整链路，且前端 / 后端 / 模型服务三层解耦、可独立演进。
...
- **可解释、可审计** — 推荐结果可追溯每条规则，便于商家调整商品策略，也利于答辩中讲清「AI 如何决策」。
